...
BOOST_FIXTURE_TEST_CASE( wrap_with_msig, eosio_wrap_tester ) try {
   ...
   trx_trace = push_action( N(rem.msig), N(exec), N(alice), mvo()
                              ("proposer",      "carol")
                              ("proposal_name", "first")
                              ("executer",      "alice")
   );
   ...
   BOOST_REQUIRE_EQUAL( name{N(rem.wrap)}, action_name{trx_trace->action_traces.at(1).receiver} );
   BOOST_REQUIRE_EQUAL( name{N(rem.wrap)}, name{trx_trace->action_traces.at(1).act.account} );
   ...
   BOOST_REQUIRE_EQUAL( name{N(rem.wrap)}, name{trx_trace->action_traces.at(1).act.authorization[1].actor} );
