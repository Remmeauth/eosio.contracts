...
public:
   ...
   eosio_system_tester(Lambda setup) {
...

      {
         ...
         rem_token_abi_ser.set_abi(abi, abi_serializer_max_time);
      }
   ...
   asset get_balance( const account_name& act ) {
      ...
      return data.empty() ? asset(0, symbol(CORE_SYMBOL)) : rem_token_abi_ser.binary_to_variant("account", data, abi_serializer_max_time)["balance"].as<asset>();
   }
   ...
   fc::variant get_stats( const string& symbolname ) {
      ...
      return data.empty() ? fc::variant() : rem_token_abi_ser.binary_to_variant( "currency_stats", data, abi_serializer_max_time );
   }
...
//                                 );
   ...
   abi_serializer rem_token_abi_ser;
};
